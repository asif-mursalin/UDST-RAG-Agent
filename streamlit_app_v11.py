...
def fetch_and_process_policy(url, policy_name):
    ...
    # Clear cache for this policy if force refresh is enabled
    force_refresh = st.sidebar.checkbox("Force Refresh Content", value=False)
...
def mistral_completion(prompt):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if st.sidebar.checkbox("Use large model (higher quality but more rate limits)", value=False):
...
if process_button and selected_policies:
    ...
    if new_policies or st.sidebar.checkbox("Force Reprocess", value=False):
        st.session_state.all_chunks = []
        st.session_state.all_chunks_with_metadata = []
        st.session_state.processed_policies = set()
        st.session_state.embeddings_loaded = False
...
k_results = st.sidebar.slider("Number of chunks to retrieve", min_value=1, max_value=10, value=5)
